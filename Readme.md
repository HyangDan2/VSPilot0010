...
The current app supports three mix styles:
...
- Zig-zag: alternates pixels on a diagonal mask and blends selected BGR channels from the second source into the first source.

## Features

- Load two independent video or image sources.
- Mix sources in real time through a dedicated mixing thread.
- Switch between odd/even column, checker-board, and zig-zag mixing.
...
- Mixing > Zig-zag: use the zig-zag channel mask.
